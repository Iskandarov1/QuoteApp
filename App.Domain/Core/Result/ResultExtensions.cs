...
namespace App.Domain.Core.Result
{
    ...
    public static class ResultExtensions
    {
        ...
        public static Result<T> Ensure<T>(this Result<T> result, Func<T, bool> predicate, Error error)
        {
            ...
            return result.IsSuccess && predicate(result.Value) ? result : App.Domain.Core.Result.Result.Failure<T>(error);
        }
        ...
        public static Result<TOut> Map<TIn, TOut>(this Result<TIn> result, Func<TIn, TOut> func) =>
            result.IsSuccess ? func(result.Value) : App.Domain.Core.Result.Result.Failure<TOut>(result.Error);
        ...
        /// </returns>
        public static async Task<App.Domain.Core.Result.Result> Bind<TIn>(this Result<TIn> result, Func<TIn, Task<App.Domain.Core.Result.Result>> func) =>
            result.IsSuccess ? await func(result.Value) : App.Domain.Core.Result.Result.Failure(result.Error);
        ...
        public static async Task<Result<TOut>> Bind<TIn, TOut>(this Result<TIn> result, Func<TIn, Task<Result<TOut>>> func) =>
            result.IsSuccess ? await func(result.Value) : App.Domain.Core.Result.Result.Failure<TOut>(result.Error);
        ...
        /// </returns>
        public static async Task<T> Match<T>(this Task<App.Domain.Core.Result.Result> resultTask, Func<T> onSuccess, Func<Error, T> onFailure)
        {
            App.Domain.Core.Result.Result result = await resultTask;
